/*
    ...
    Copyright (c) 2011 STRAIGHTLINE<http://www.straightline.jp/> All rights reserved.
*/
...
var Ajax = new Class({
            ...
            if (element && element.retrieve('refElement') != null) {
                ...
                var request = new Request({
                    ...
                    onSuccess: function(text, xml) {
                        ...
                        if (hash == '' || hash == '/') {
                            if (this.hashListner) {
                                this.hashListner.currentHash = '';
                                this.hashListner.setHash('');
                            }
                        } else if (hash) {
                            if (this.hashListner) {
                                this.hashListner.currentHash = hash;
    ...
    run: function() {
        var ajax = new Array();
        if (this.options.ajaxSelector) {
            ajax = $$(this.options.ajaxSelector);
        } else {
            ajax = $$('.ajax');
        }
        ...
        if (this.options.ajaxFormSelector) {
            ajaxForm = $$(this.options.ajaxFormSelector);
        } else {
            ajaxForm = $$('form.ajax');
        }
...
var PopupImage = new Class({
    ...
    loadImage: function(href, element) {
        ...
        if (this.loader == null) {
            this.loader = new Element('div', { 'id': 'popup-img-loader', styles: { opacity: 0 } });
            ...
            if (element && this.imgAnchors.length == 0) {
                ...
                if (group) {
                    ...
                    if (this.options.allAnchorScan) {
                        ...
                        groupElements = group.getElements('img');
                    }
                    groupElements.each(function(curElement) {
                        ...
                        if (anchor) {
                            ...
                            if (curHref.match(/(gif|jpg|png)/i)) {
                ...
                onComplete: function() {
                    ...
                    var imgLoaded = function() {
                        ...
                        if (element) {
                            ...
                            if (parent) {
                                var description = parent.getElement(this.options.popupDescriptionSelector);
                                if (description) {
                                    description = new Element('div', {
                                        'class': 'popup-description',
                                        html: description.get('html')
                                    });
                                    this.imgWrapper.adopt(description);
                                }
                            }
                        }
...
onFirstLoaded: function(){},
                                    ...
                                    slideInner.addEvents({
                                        ...
                                        touchmove: function(event) {
                                            ...
                                            if (slide.retrieve('isSliding') == null) {
                                                var touch = event.touches[0];
                                                if (Math.abs(startY - touch.pageY) > 100) {
                                                    skipSlide = true;
                                                }
                                                if (
                                                    (Math.abs(startX - touch.pageX) > 30 || skipDefault) &&
                                                    skipSlide == false
                                                ) {
                                                    skipDefault = true;
    ...
    scanSlideItem: function(slide) {
        ...
        this.slide.apply(this, [slide, index, false, true]);
    },

    setLoopSlideItem: function(slide, targetIndex) {
        var slideItems = slide.retrieve('slideItems');
        var slideInner = slide.retrieve('slideInner');
    ...
    slide: function(slide, index, isFirst, stopDragNext) {
        if (slide.retrieve('isSliding') == null) {
            slide.store('isSliding', true);
            
            if (this.options.animationType == this.animationType.loopHSlide) {
                index = this.setLoopSlideItem.apply(this, [slide, index]);
            }
            ...
            if (targetSlideItem) {
                ...
                var imgLoaded = function() {
                    ...
                    if (this.options.animationType == this.animationType.fade) {
                        ...
                        if (slide.retrieve('slideItems')[curIndex]) {
                            new Fx.Tween(slide.retrieve('slideItems')[curIndex], {
                                property: 'opacity',
                                duration: this.options.fadeDuration,
                                transition: this.options.fadeTransition
                            }).start(0);
                    ...
                    if (slide.retrieve('slideItems')[curIndex]) {
                        var curSlidePageNo = slide.retrieve('slideItems')[curIndex].retrieve('slidePageNo');
                        if (curSlidePageNo) {
...
var Background = new Class({
    ...
    run: function() {
        ...
        this.container.store('mousemoveEvent', function(event) {
            ...
            if (this.backgroundImages.length > 0) {
                ...
                if (img.hasClass('background-trim') && img.retrieve('animation') == null) {
                    ...
                    var rateX = Math.abs((imgSize.x - windowSize.x) / windowSize.x);
                    var rateY = Math.abs((imgSize.y - windowSize.y) / windowSize.y);
                    img.setStyles({
                        'margin-top': - rateY * event.client.y,
                        'margin-left': - rateX * event.client.x
                    });
                }
            }
        }.bind(this));
        $(document.body).addEvent('mousemove', this.container.retrieve('mousemoveEvent'));
